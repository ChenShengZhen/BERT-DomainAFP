...
learner = PretrainingWrapper(
    model,
    random_replace_token_prob = 0.05,    # Replace with a random token percentage, default to 5% in the paper
    remove_annotation_prob = 0.25,       # Delete annotation percentage, default to 25%
    add_annotation_prob = 0.01,          # The probability of adding annotations randomly, default to 1%
    remove_all_annotations_prob = 0.5,   # The percentage of completely deleting annotations in batch projects defaults to 50%
    seq_loss_weight = 1.,                # Weight of sequence loss
    annotation_loss_weight = 1.,         # The weight of annotation loss
    exclude_token_ids = ()        # Used to exclude padding, starting, and ending tokens from being masked
)
...
def test_model(model, test_loader):  
    ...
    print(f"model acc: {accuracy * 100:.2f}%")  
    print(f"type acc: {AFP_total,AFP_predict}")
    return accuracy
    ...
    for type_value in unique_types:  
        ...
        if len(type_df) < target_count:  
            type_df_upsampled = resample(type_df,   
                                          replace=True,    # Allow duplicate sampling
                                          n_samples=target_count, # Sampling to target quantity
                                          # Random_state can be set to obtain reproducible results
                                          #random_state=123
                                          )  
...
for epoch in range(num_epochs):  
    ...
    if right > best_acc:
        ...
        best_acc=right
    print(f"loss{val_loss},acc{right}")
